...
function generatePillHtml(targetName, targetColorClass, traineeNric, volNric) {
  return `<div class="relative inline-block m-1.5 align-top pointer-events-auto">
      <div class="${targetColorClass} text-[9px] md:text-[10px] pl-1.5 pr-2.5 py-0.5 rounded border border-gray-300 dark:border-gray-600 font-semibold shadow-sm flex items-center justify-center truncate max-w-[85px] sm:max-w-[120px] opacity-80">
          ${targetName}
      </div>
  ...
  return `
    <div class="dnd-draggable dnd-dropzone bg-white dark:bg-gray-800 p-2 md:p-3 rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col min-h-[70px] gap-1.5" data-nric="${item.nric}" data-role="${item.role}">
      ...
      <div class="flex justify-between items-start w-full gap-1">
        <span class="main-name-pill font-extrabold text-[13px] md:text-[14px] px-2.5 py-1 rounded-md border shadow-sm ${dynColor} truncate max-w-full tracking-tight inline-flex items-center self-start">${item.name} ${famBadge}</span>
...
trainees.forEach(t => {
 ...
 const cardHtml = `<div class="bg-white dark:bg-gray-800 p-3 md:p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-2 transition">
       <div class="flex justify-between items-start mb-1"><div class="flex items-center"><span class="font-extrabold text-sm md:text-base px-2 py-0.5 rounded border shadow-sm ${dynColor}">${t.name}</span>${famBadge}</div><button onclick="openPairingSheet('${t.nric}', 'TRAINEE')" class="text-[10px] md:text-xs bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400 font-bold px-2 py-1 rounded-md border border-blue-200 dark:border-gray-600 hover:bg-blue-100 transition whitespace-nowrap focus:outline-none">+ Vol</button></div>
 ...
 targets.forEach(t => {
   ...
   html += `<div onclick="confirmPairing('${t.nric}')" class="pairing-list-item flex flex-col bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 transition mb-2" data-name="${t.name.toLowerCase()}">
       <div class="flex justify-between items-center w-full">
         <span class="font-bold text-sm px-2 py-0.5 rounded border ${tDynColor}">${t.name}</span>
         <span class="text-[10px] font-semibold ${roleColor} border px-1.5 py-0.5 rounded">${roleLabel}</span>
       </div>
     </div>`;
 });
