...
<div id="log-pairings" class="flex-1 flex flex-col min-h-0 mt-2 relative">
  ...
  <div class="bg-white dark:bg-gray-800 p-2 md:p-3 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col flex-1 min-h-0">
    ...
    <div class="flex flex-row gap-2 flex-1 min-h-0 w-full overflow-hidden">
      ...
      <div id="dnd-source-col" class="flex-1 min-w-0 rounded-xl border flex flex-col h-full overflow-hidden transition-colors">
      ...
      <div id="dnd-target-col" class="flex-1 min-w-0 rounded-xl border flex flex-col h-full overflow-hidden transition-colors">
...
function generatePillHtml(targetName, targetColorClass, traineeNric, volNric) {
 return `<div class="relative inline-block m-1.5 align-top pointer-events-auto">
     <div class="${targetColorClass} text-[9px] md:text-[10px] pl-1.5 pr-2.5 py-0.5 rounded border border-gray-300 dark:border-gray-600 font-semibold shadow-sm flex items-center justify-center max-w-[85px] sm:max-w-[120px] opacity-80 leading-tight break-words whitespace-normal text-center" style="overflow-wrap: anywhere;">
         ${targetName}
     </div>
...
function generateCardHtml(item, familyCounts, pairings, vols, trainees) {
 ...
 const famBadge = isFam && item.role === 'TRAINEE' ? `<span class="bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded shrink-0 shadow-sm border border-purple-200 dark:border-purple-700 pointer-events-none whitespace-nowrap">FAM</span>` : '';
 ...
 const btnLabel = item.role === 'TRAINEE' ? '+ Vol' : '+ Trainee';

 return `
   <div class="dnd-draggable dnd-dropzone bg-white dark:bg-gray-800 p-2 md:p-3 rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col min-h-[70px] gap-1.5" data-nric="${item.nric}" data-role="${item.role}">
     <!-- Unique Targetable class for cloning -->
     <div class="flex justify-between items-start w-full gap-1.5">
       <div class="main-name-pill font-extrabold text-[12px] md:text-[13px] px-2 py-1 rounded-md border shadow-sm ${dynColor} max-w-full tracking-tight inline-flex flex-wrap items-center gap-1.5 self-start min-w-0 leading-tight">
         <span class="break-words whitespace-normal min-w-0 text-left" style="overflow-wrap: anywhere;">${item.name}</span>
         ${famBadge}
       </div>
       <button onclick="openPairingSheet('${item.nric}', '${item.role}')" class="text-[9px] md:text-[10px] bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400 font-bold px-1.5 py-1 rounded-md border border-blue-200 dark:border-gray-600 hover:bg-blue-100 transition whitespace-nowrap focus:outline-none shrink-0 pointer-events-auto shadow-sm">${btnLabel}</button>
     </div>
     <div class="flex flex-wrap flex-grow items-start content-start pointer-events-auto bg-gray-50/50 dark:bg-gray-900/50 p-1 rounded min-h-[30px] border border-dashed border-gray-200 dark:border-gray-700">
...
const volColClass = "bg-green-50/60 dark:bg-green-900/20 border-green-300 dark:border-green-800";
const traineeColClass = "bg-blue-50/60 dark:bg-blue-900/20 border-blue-300 dark:border-blue-800";
...
sourceCol.className = `flex-1 min-w-0 rounded-xl border flex flex-col h-full overflow-hidden transition-colors ${sourceColClass}`;
targetCol.className = `flex-1 min-w-0 rounded-xl border flex flex-col h-full overflow-hidden transition-colors ${targetColClass}`;
...
sourceTitle.className = `font-extrabold text-[12px] md:text-sm py-2 shrink-0 text-center uppercase tracking-widest shadow-sm rounded-t-md border-b-2 ${isSourceVol ? volTitleClass : traineeTitleClass}`;
targetTitle.className = `font-extrabold text-[12px] md:text-sm py-2 shrink-0 text-center uppercase tracking-widest shadow-sm rounded-t-md border-b-2 ${!isSourceVol ? volTitleClass : traineeTitleClass}`;
...
if (sourcePerson) {
  ...
  titleHtml = `Pair with <span class="ml-1.5 font-bold text-sm px-2 py-0.5 rounded border ${dynColor}">${sourcePerson.name}</span>`;
}
...
targets.forEach(t => {
  ...
  const roleColor = t.role === 'VOLUNTEER' ? 'text-green-600 bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800' : 'text-blue-600 bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800';
  
  html += `<div onclick="confirmPairing('${t.nric}')" class="pairing-list-item flex flex-col bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 transition mb-2" data-name="${t.name.toLowerCase()}">
      <div class="flex justify-between items-start w-full gap-2">
        <span class="font-bold text-sm px-2 py-1 rounded border ${tDynColor} break-words whitespace-normal min-w-0 flex-1 text-left leading-tight" style="overflow-wrap: anywhere;">${t.name}</span>
        <span class="text-[10px] font-semibold ${roleColor} border px-1.5 py-0.5 rounded shrink-0 whitespace-nowrap mt-0.5">${roleLabel}</span>
      </div>
    </div>`;
});

document.getElementById('sheetListContainer').innerHTML = html || `<p class="text-sm font-medium text-gray-400 p-2">No available ${targetRole.toLowerCase()}s.</p>`;
}
