...
window.AppCore = {
   ...
   renderHeader: function(pageTitle, backUrl = './dashboard.html') {
       const headerHtml = `
       <header class="sticky top-0 z-50 shadow-sm bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 flex items-center justify-between transition-colors w-full">
           <div class="flex items-center min-w-0 flex-1">
               ${backUrl ? `<a href="${backUrl}" class="mr-3 p-1.5 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition focus:outline-none shrink-0"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7" /></svg></a>` : ''}
               <div class="flex flex-col min-w-0">
                   <h1 class="font-black text-lg md:text-xl text-zinc-900 dark:text-white leading-tight truncate tracking-tight">${pageTitle}</h1>
                   <span id="headerSubtitle" class="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider truncate"></span>
               </div>
           </div>
           <div class="flex items-center gap-1 shrink-0 ml-2">
               <button onclick="AppCore.hardRefresh()" class="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition focus:outline-none" title="Refresh Cache">
               ...
               <button onclick="AppCore.toggleTheme()" class="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition focus:outline-none" title="Toggle Theme">🌗</button>
               <a href="./dashboard.html" class="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition focus:outline-none" title="Home">
