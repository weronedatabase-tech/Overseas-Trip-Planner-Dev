...
function openFinanceRatesModal() {
   ...
   let html = '<p class="text-[10px] text-zinc-500 dark:text-zinc-400 mb-3 leading-tight">Override live exchange rates. Value represents 1 foreign unit in SGD.</p>';
   Object.keys(globalFinanceRates).forEach(c => {
       ...
       html += `
       <div class="flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50 p-2 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
           <div class="font-black text-xs text-zinc-800 dark:text-zinc-200 w-16 text-center shrink-0">1 ${c}</div>
           <div class="font-bold text-xs text-zinc-400 px-2 shrink-0">=</div>
           <div class="flex-1 min-w-0 pr-2">
               <input type="number" step="0.0001" placeholder="Live: ${live.toFixed(4)}" value="${custom}" 
                   onchange="setCustomRate('${c}', this.value)" 
                   class="w-full text-sm font-bold p-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-950 focus:outline-none focus:border-primary focus:ring-1 text-zinc-900 dark:text-white transition shadow-sm placeholder-zinc-400">
           </div>
           <div class="font-black text-xs text-zinc-800 dark:text-zinc-200 shrink-0">SGD</div>
       </div>`;
   });
   list.innerHTML = html;
...
async function buildFinanceUI() {
   ...
   <div class="flex flex-col h-full w-full relative">
     <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-2 md:p-3 shrink-0 flex justify-between items-center z-10 shadow-sm rounded-t-xl md:rounded-none">
         <div class="flex items-center gap-2">
             <h3 class="text-sm md:text-base font-black text-zinc-900 dark:text-white tracking-tight">Finance Options Planner</h3>
         </div>
         <div class="flex items-center gap-1.5 md:gap-2">
             <button onclick="addFinanceOption()" class="bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 text-[10px] md:text-xs font-bold px-2 py-1.5 rounded-md hover:bg-blue-100 transition shadow-sm focus:outline-none shrink-0">+ Add Option</button>
             <button id="btn-sync-finance" onclick="manualFinanceSync(this)" class="bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 text-[10px] md:text-xs font-bold px-3 py-1.5 rounded-md hover:bg-green-100 transition flex items-center shadow-sm focus:outline-none shrink-0 border"><span class="btn-text">Saved</span><div class="btn-spinner ml-1.5 !w-3 !h-3 hidden-force border-2"></div></button>
         </div>
     </div>

     <div id="financeGlobalSettings" class="bg-white dark:bg-zinc-800 p-2 md:p-3 shrink-0 border-b border-zinc-200 dark:border-zinc-800 flex flex-wrap items-center gap-2 z-10 shadow-[0_4px_10px_-5px_rgba(0,0,0,0.05)]"></div>
     ...
     <div id="financeLoadingOverlay" class="absolute inset-0 bg-white/60 dark:bg-zinc-950/60 backdrop-blur-sm z-[50] flex flex-col justify-center items-center">
         ...
         <span class="text-primary dark:text-blue-400 font-bold text-[10px] tracking-wide shadow-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-1 rounded-full">Loading Planner...</span>
     </div>

     <div class="flex-grow overflow-y-auto overflow-x-hidden p-2 md:p-4 bg-zinc-50 dark:bg-zinc-950 custom-scrollbar pb-10">
         <div id="financeOptionsContainer" class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4 w-full items-start pb-4 max-w-full mx-auto"></div>
     </div>
     ...
     <div id="financeRatesModal" class="fixed inset-0 bg-black/60 z-[96] hidden-force flex justify-center items-center p-4 backdrop-blur-sm transition-opacity">
         <div class="bg-white dark:bg-zinc-800 rounded-2xl w-full max-w-sm p-5 shadow-2xl border border-zinc-200 dark:border-zinc-700 m-auto animate-slide-up flex flex-col max-h-[90vh]">
             <div class="flex justify-between items-center mb-3 border-b border-zinc-200 dark:border-zinc-700 pb-2 shrink-0">
                 <h3 class="text-base font-black text-zinc-900 dark:text-white flex items-center gap-2"><svg class="w-5 h-5 text-blue-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> Exchange Rates</h3>
             ...
             <div class="mt-3 pt-3 border-t border-zinc-200 dark:border-zinc-700 shrink-0 flex justify-end">
   ...
   container.innerHTML = `
     <div class="flex flex-wrap md:flex-nowrap justify-between items-center gap-2 w-full">
         <div class="flex flex-wrap items-center gap-2 flex-1">
             <div class="flex items-center gap-1.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-2 py-1">
                 <label class="text-[10px] uppercase font-bold text-zinc-500 dark:text-zinc-400 tracking-wider shrink-0">Pax Mode:</label>
                 <select onchange="updateFinanceConfig('globalPaxMode', this.value)" class="text-[11px] font-bold bg-transparent text-zinc-900 dark:text-white focus:outline-none cursor-pointer">
                     <option value="individual" ${financeConfig.globalPaxMode === 'individual' ? 'selected' : ''}>Manual Override (Individual)</option>
                     <option value="manual" ${financeConfig.globalPaxMode === 'manual' ? 'selected' : ''}>Manual Override (Global)</option>
                     <option value="auto" ${financeConfig.globalPaxMode === 'auto' ? 'selected' : ''}>Total Pax (Auto)</option>
                 </select>
             </div>
             <div class="flex items-center gap-1.5 ${financeConfig.globalPaxMode !== 'manual' ? 'hidden-force' : ''}">
                 ...
                 <input type="number" min="0" value="${financeConfig.globalPaxCount}" onchange="updateFinanceConfig('globalPaxCount', this.value)" onkeyup="updateFinanceConfig('globalPaxCount', this.value)" class="hide-spinners w-16 text-xs font-bold border border-zinc-300 dark:border-zinc-600 rounded px-1.5 py-1 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary shadow-sm text-center">
             </div>
         ...
         <div class="flex gap-2">
             <button onclick="openFinanceRatesModal()" class="text-[10px] md:text-xs font-bold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 px-2.5 py-1.5 rounded shadow-sm whitespace-nowrap shrink-0 transition focus:outline-none flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> Rates</button>
             <button onclick="toggleFinanceCollapse()" class="text-[10px] md:text-xs font-bold text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 px-2.5 py-1.5 rounded shadow-sm whitespace-nowrap shrink-0 transition focus:outline-none">${isFinanceCollapsed ? 'Expand All' : 'Collapse All'}</button>
         </div>
     </div>
   `;
}
...
function startFinDrag(e) {
   ...
   row.style.position = 'fixed'; row.style.zIndex = '9999'; row.style.width = rect.width + 'px'; row.style.boxShadow = '0 10px 25px rgba(0,0,0,0.2)'; row.classList.add('opacity-95');
...
function endFinDrag(e) {
   ...
   finDndState.placeholder.remove(); finDndState.row.style = ''; finDndState.row.classList.remove('opacity-95');
...
function renderFinanceOptions() {
   ...
   financeOptions.forEach(opt => {
       ...
       html += `
       <div class="w-full shrink-0 flex flex-col bg-white dark:bg-zinc-800 rounded-xl shadow-md border border-zinc-200 dark:border-zinc-700 overflow-hidden h-fit transition-all duration-300 ${spanClass}">
           <div class="p-2 md:p-3 bg-zinc-50/80 dark:bg-zinc-900/50 flex justify-between items-center gap-2 shrink-0 ${isLocalCollapsed ? '' : 'border-b border-zinc-200 dark:border-zinc-700'}">
               <input type="text" value="${opt.title}" onchange="updateFinanceOption('${opt.id}', 'title', this.value)" class="font-black text-base md:text-lg bg-transparent border-b border-transparent focus:border-primary outline-none text-zinc-900 dark:text-white flex-1 min-w-0 px-1 transition pb-0.5">
               <div class="flex items-center gap-1.5 shrink-0">
                   <button onclick="cycleFinanceOptionWidth('${opt.id}')" class="hidden lg:block text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 p-1 md:p-1.5 rounded bg-zinc-100/50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 focus:outline-none" title="Toggle Width"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 8L4 12l4 4m8-8l4 4-4 4"></path></svg></button>
                   <button onclick="toggleIndividualFinanceCollapse('${opt.id}')" class="text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200 p-1 md:p-1.5 rounded bg-zinc-100/50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 focus:outline-none"><svg class="w-4 h-4 transform transition-transform ${isLocalCollapsed ? '' : 'rotate-180'}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg></button>
                   <button onclick="duplicateFinanceOption('${opt.id}')" class="text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 p-1.5 rounded" title="Duplicate"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>
                   <button onclick="removeFinanceOption('${opt.id}')" class="text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 p-1.5 rounded" title="Delete"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
               </div>
           </div>
           <div class="${isLocalCollapsed ? 'hidden-force' : 'flex flex-col'}">
               <div class="px-2 md:px-3 py-2 bg-white dark:bg-zinc-800 border-b border-zinc-100 dark:border-zinc-700 flex justify-between items-center">
                   <label class="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Pax Count ${paxInputDisabled ? '(Global)' : ''}</label>
                   <input type="number" min="0" value="${pax}" ${paxInputDisabled ? 'disabled' : ''} onchange="updateFinanceOption('${opt.id}', 'pax', this.value)" onkeyup="updateFinanceOption('${opt.id}', 'pax', this.value)" class="hide-spinners w-20 text-xs font-bold px-2 py-1 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-600 rounded text-center focus:outline-none focus:ring-1 focus:ring-primary ${paxInputDisabled ? 'opacity-60 bg-zinc-100 dark:bg-zinc-800' : ''}">
               </div>
               <div class="fin-cat-container p-2 bg-white dark:bg-zinc-800 flex flex-col gap-2 max-h-[50vh] overflow-y-auto custom-scrollbar" data-opt-id="${opt.id}">
                   ${opt.fields.map(f => {
                       ...
                       <div class="fin-cat-row flex flex-col w-full bg-zinc-50 dark:bg-zinc-900/50 p-2 rounded-lg border border-transparent focus-within:border-zinc-300 dark:focus-within:border-zinc-600 transition shadow-sm" data-field-id="${f.id}">
                           <div class="flex items-center gap-2 w-full mb-1">
                               ...
                               <input type="text" value="${f.name}" onchange="updateFinanceField('${opt.id}', '${f.id}', 'name', this.value)" class="flex-1 min-w-[80px] bg-transparent text-sm font-bold text-zinc-900 dark:text-zinc-100 outline-none px-1 border-b border-transparent focus:border-primary transition" placeholder="Category Name">
                           </div>
                           <div class="flex items-center flex-wrap gap-2 pl-[42px] w-full">
                               <select onchange="updateFinanceField('${opt.id}', '${f.id}', 'currency', this.value)" class="w-[65px] shrink-0 bg-white dark:bg-zinc-950 text-xs font-bold border border-zinc-300 dark:border-zinc-600 rounded py-1.5 pl-1.5 pr-0 outline-none focus:border-primary cursor-pointer">${getCurrencyOptions(f.currency)}</select>
                               <select onchange="updateFinanceField('${opt.id}', '${f.id}', 'costType', this.value)" class="w-[70px] shrink-0 text-xs font-extrabold border rounded py-1.5 px-1 outline-none cursor-pointer ${costTypeColorClass}"><option value="total" ${f.costType !== 'per_pax' ? 'selected' : ''}>Total</option><option value="per_pax" ${f.costType === 'per_pax' ? 'selected' : ''}>/Pax</option></select>
                               <input type="text" value="${displayCostStr}" oninput="formatMoneyInput(this, false); updateFinanceField('${opt.id}', '${f.id}', 'cost', this.value)" onblur="formatMoneyInput(this, true); updateFinanceField('${opt.id}', '${f.id}', 'cost', this.value)" class="w-[100px] shrink-0 bg-white dark:bg-zinc-950 text-sm font-bold border border-zinc-300 dark:border-zinc-600 rounded px-2 py-1 outline-none focus:border-primary text-right" placeholder="0.00">
                               <div class="flex items-center gap-1 w-[70px] shrink-0 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-600 rounded px-1.5 py-1 outline-none focus-within:border-primary"><span class="text-[10px] font-bold text-zinc-400">+</span><input type="number" step="0.1" min="0" value="${f.tax || ''}" onchange="updateFinanceField('${opt.id}', '${f.id}', 'tax', this.value)" onkeyup="updateFinanceField('${opt.id}', '${f.id}', 'tax', this.value)" class="hide-spinners w-full bg-transparent text-sm font-bold outline-none text-right" placeholder="Tax"><span class="text-[10px] font-bold text-zinc-500">%</span></div>
                               <input type="text" value="${f.remarks}" onchange="updateFinanceField('${opt.id}', '${f.id}', 'remarks', this.value)" class="flex-1 min-w-[120px] bg-transparent text-xs font-medium text-zinc-500 dark:text-zinc-400 outline-none px-1 border-b border-transparent focus:border-primary transition" placeholder="Remarks...">
                           </div>
                       </div>`;
                   }).join('')}
                   <div class="pt-2 px-1"><button onclick="addFinanceCategory('${opt.id}')" class="w-full py-2 border border-dashed border-blue-300 dark:border-blue-800 rounded-lg text-blue-600 dark:text-blue-400 text-xs font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 transition">+ Add Custom Category</button></div>
               </div>
           </div>
           <div class="p-2 md:p-3 bg-blue-50/80 dark:bg-blue-900/20 shrink-0 flex flex-col gap-1.5 ${isLocalCollapsed ? 'border-t border-zinc-200 dark:border-zinc-700' : 'border-t border-blue-100 dark:border-blue-900/50'}">
               <div class="${isLocalCollapsed ? 'hidden-force' : 'flex'} justify-between items-center pb-2 border-b border-blue-200/50 dark:border-blue-800/50 mb-1">
                   ...
                   <select onchange="updateFinanceOption('${opt.id}', 'displayCurrency', this.value)" class="w-[90px] text-xs font-bold px-2 py-1 bg-white dark:bg-zinc-950 border border-blue-300 dark:border-blue-700 rounded focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer shadow-sm text-blue-900 dark:text-blue-100">${getCurrencyOptions(opt.displayCurrency)}</select>
               </div>
               <div class="flex justify-between items-center"><span class="font-black text-xs md:text-sm text-blue-800 dark:text-blue-300 uppercase tracking-widest">Total Estimated</span><span id="total_${opt.id}" class="font-black text-base md:text-lg text-blue-700 dark:text-blue-400 bg-white dark:bg-zinc-900 px-2 py-1 rounded border border-blue-200 dark:border-blue-800 shadow-sm leading-none">${opt.displayCurrency} ${totalDisp.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span></div>
               <div class="flex justify-between items-center"><span class="font-black text-xs md:text-sm text-emerald-800 dark:text-emerald-400 uppercase tracking-widest">Cost Per Pax</span><span id="cpp_${opt.id}" class="font-black text-base md:text-lg text-emerald-700 dark:text-emerald-400 bg-white dark:bg-zinc-900 px-2 py-1 rounded border border-emerald-200 dark:border-emerald-800 shadow-sm leading-none">${opt.displayCurrency} ${cppDisp.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span></div>
           </div>
       </div>`;
   });
