...
<div class="sticky top-0 z-40 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 p-2 md:p-3 shrink-0 flex flex-col gap-2 shadow-sm rounded-t-xl md:rounded-none">
  ...
  <div class="relative">
     <input type="text" id="attSearchInput" oninput="handleAttendanceSearch()" placeholder="Search to mark present..." class="w-full p-1.5 pl-8 pr-8 border border-gray-300 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-800 font-bold text-xs focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-gray-900 dark:text-white shadow-sm transition">
...
document.addEventListener('click', (e) => {
  ...
  if(results && !results.classList.contains('hidden-force') && e.target !== input && !results.contains(e.target) && !e.target.closest('button[onclick*="clearSearch"]')) {
...
clearSearch('attSearchInput', 'handleAttendanceSearch');
