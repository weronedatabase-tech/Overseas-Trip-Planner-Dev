...
function buildAttendanceUI() {
   ...
   <div class="admin-only flex flex-col h-full min-h-0 w-full relative">
       ...
       <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-2 md:p-3 shrink-0 z-10 flex flex-col gap-2 shadow-sm rounded-t-xl md:rounded-none">
           <div class="flex justify-between items-center">
               <h3 class="text-sm md:text-base font-black text-zinc-900 dark:text-white tracking-tight">Live Attendance</h3>
               <button id="btn-sync-attendance" onclick="manualSyncAttendance()" class="text-[10px] md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-sm bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0">
                   <span class="btn-text">Saved</span><div class="btn-spinner ml-1 !w-3 !h-3 hidden-force border-2"></div>
               </button>
           </div>
           
           <div class="grid grid-cols-2 gap-2">
               <div class="flex gap-1">
                   <select id="attJunctureSelect" onchange="changeAttendanceContext()" class="w-full p-1.5 border border-zinc-300 dark:border-zinc-700 rounded-md font-bold text-xs bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary shadow-sm appearance-none truncate"></select>
               ...
               <select id="attAssignmentSelect" onchange="renderAttendanceLists()" class="w-full p-1.5 border border-zinc-300 dark:border-zinc-700 rounded-md font-bold text-xs bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary shadow-sm appearance-none truncate">
                   <option value="ALL">All Participants</option>
               </select>
           </div>
           
           <div class="relative">
               <input type="text" id="attSearchInput" oninput="handleAttendanceSearch()" placeholder="Search to mark present..." class="w-full p-1.5 pl-8 border border-zinc-300 dark:border-zinc-700 rounded-md bg-zinc-50 dark:bg-zinc-800 font-bold text-xs focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-zinc-900 dark:text-white shadow-sm transition">
               ...
               <ul id="attSearchResults" class="absolute z-20 w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md shadow-2xl mt-1 max-h-56 overflow-y-auto hidden-force custom-scrollbar"></ul>
           </div>
       </div>

       <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative bg-zinc-50 dark:bg-zinc-950 rounded-b-xl md:rounded-none border-x border-b border-zinc-200 dark:border-zinc-800">
...
function renderAttendanceLists() {
   ...
   notCheckedList.innerHTML = notCheckedHtml || '<p class="text-[10px] text-zinc-400 dark:text-zinc-500 font-bold p-2 text-center mt-2">Empty</p>';
   checkedList.innerHTML = checkedHtml || '<p class="text-[10px] text-zinc-400 dark:text-zinc-500 font-bold p-2 text-center mt-2">Empty</p>';
...
function generateAttCard(p, isChecked) {
   ...
   return `
   <div id="att-card-${p.nric}" class="relative bg-white dark:bg-zinc-800 p-1.5 md:p-2 rounded border border-zinc-200 dark:border-zinc-700 shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-all duration-300 flex items-center justify-between gap-1 select-none active:scale-95 cursor-pointer hover:border-primary dark:hover:border-primary" onclick="toggleAttendanceStatus('${p.nric}', ${!isChecked})">
       <div class="flex items-start min-w-0 flex-1">
           <div class="flex flex-col min-w-0 flex-1 gap-1">
               <span class="font-extrabold text-[11px] md:text-[12px] px-1.5 py-0.5 rounded border ${dynColor} max-w-full break-words whitespace-normal leading-[1.1] text-left">${p.displayName || p.name}</span>
               <span class="text-[8px] font-black ${roleColor} w-max bg-zinc-50 dark:bg-zinc-700 px-1 py-0.5 rounded uppercase border border-zinc-100 dark:border-zinc-600">${p.role.substring(0,3).toUpperCase()}</span>
           </div>
       </div>
       <div class="shrink-0 flex items-center justify-center pl-1">
          <div class="w-5 h-5 rounded flex items-center justify-center border transition-colors ${isChecked ? 'bg-green-500 border-green-600 text-white' : 'bg-zinc-100 border-zinc-300 dark:bg-zinc-700 dark:border-zinc-600 text-transparent'}">
...
function handleAttendanceSearch() {
   ...
   resultsContainer.innerHTML = participants.map(p => {
       ...
       const dynColor = typeof window.getProjectColor === 'function' ? window.getProjectColor(p.group) : 'bg-zinc-100';
       return `
       <li class="px-2 py-1.5 hover:bg-zinc-50 dark:hover:bg-zinc-700 cursor-pointer flex justify-between items-center border-b border-zinc-100 dark:border-zinc-700 transition" onclick="selectFromSearch('${p.nric}')">
          <span class="font-bold text-[11px] md:text-xs ${dynColor} px-1.5 py-0.5 rounded-md border max-w-[70%] break-words">${p.displayName || p.name}</span>
          ${isChecked ? '<span class="text-[9px] bg-green-100 text-green-700 border border-green-200 dark:bg-green-900/30 dark:text-green-400 px-1 py-0.5 rounded font-black uppercase">Checked</span>' : '<span class="text-[9px] bg-red-100 text-red-700 border border-red-200 dark:bg-red-900/30 dark:text-red-400 px-1 py-0.5 rounded font-black uppercase">NOT Checked</span>'}
       </li>`;
   }).join('') || '<li class="px-3 py-2 text-[10px] font-bold text-zinc-500 dark:text-zinc-400 text-center">No matches found.</li>';
