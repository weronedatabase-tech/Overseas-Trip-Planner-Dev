...
try {
 const res = await apiCall('bulkDriveOperation', { actionType, items, targetFolderId, singleNewName });
...
try {
const res = await apiCall('createDriveFolder', { parentFolderId: current.id, folderName: folderName.trim() });
renderDriveContents(res.folders, res.files);
...
try {
 const res = await apiCall('createGoogleDoc', { folderId: current.id, fileName: fileName.trim(), docType: docType });
 renderDriveContents(res.folders, res.files);
...
if (skippedFiles.length > 0) showToast(`Skipped ${skippedFiles.length} file(s) larger than 4MB.`, true);
...
try {
   ...
   const res = await apiCall('uploadDriveFile', { folderId: current.id, fileName: file.name, mimeType: file.type || 'application/octet-stream', fileData: base64Data });
   lastRes = res;
   successCount++;
} catch(err) {
...
if (successCount > 0) showToast(`Successfully uploaded ${successCount} file(s).`);
}
...
try {
const res = await apiCall('renameDriveItem', { itemId: id, isFolder: isFolder, newName: newName.trim(), currentFolderId: current.id });
renderDriveContents(res.folders, res.files);
