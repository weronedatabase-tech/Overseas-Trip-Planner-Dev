...
<div class="bg-white dark:bg-gray-900 p-3 md:p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 main-admin-only ${currentUser && currentUser.nric === 'ADMIN' ? '' : 'hidden-force'}">
 ...
 <div class="flex flex-col space-y-2 mb-3 mt-2">
   ...
   <div class="flex flex-row space-x-2 justify-end items-center">
     <select id="newDriveRole" class="p-2 border border-gray-300 dark:border-gray-700 rounded-md text-xs font-bold bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white w-full md:w-32 focus:outline-none focus:ring-1 focus:ring-primary shadow-sm">
       <option value="viewer">Viewer</option>
       <option value="editor">Editor</option>
     </select>
     
     <button onclick="massAddDriveAccessBtn(this)" class="bg-primary text-white px-3 py-2 text-xs rounded-md font-bold flex items-center justify-center shadow-sm shrink-0 w-24"><span class="btn-text">Mass Add</span><div class="btn-spinner spinner-white hidden-force ml-1.5 !w-3 !h-3 border-2"></div></button>
     
     <button onclick="massRemoveSpecificDriveAccessBtn(this)" class="bg-white dark:bg-gray-800 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/50 px-3 py-2 text-xs rounded-md font-bold flex items-center justify-center shadow-sm shrink-0 w-28 hover:bg-red-50 dark:hover:bg-gray-700 transition focus:outline-none"><span class="btn-text">Mass Remove</span><div class="btn-spinner spinner-red hidden-force ml-1.5 !w-3 !h-3 border-2"></div></button>
   </div>
 </div>
...
async function massRemoveSpecificDriveAccessBtn(btn) {
  const rawEmails = document.getElementById('newDriveEmails').value;
  const emails = parseEmails(rawEmails);
  
  if(emails.length === 0) return showToast("Enter at least one email to remove", true);
  ...
  try {
    ...
    if(res.results.failed.length > 0) {
      showToast(`Removed ${res.results.success.length}. Failed: ${res.results.failed.length}. Check console.`, true);
      console.error("Drive Access Remove Failures:", res.results.failed);
    } else {
      showToast(`Removed access for ${emails.length} user(s)`);
    }
  } catch(e) {
    showToast(e.message, true);
  } finally {
