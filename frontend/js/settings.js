...
function renderGroupList(list) {
  ...
  if(!ul) return; 
  ul.innerHTML = (!list || list.length === 0) ? '<li class="text-[10px] font-bold text-gray-500 dark:text-gray-400 px-1">No projects defined yet.</li>' : '';
...
function renderCommList(list) { 
  ...
  if(!ul) return; 
  ul.innerHTML = (!list || list.length === 0) ? '<li class="text-[10px] font-bold text-gray-500 dark:text-gray-400 px-1">No committee members assigned yet.</li>' : ''; 
