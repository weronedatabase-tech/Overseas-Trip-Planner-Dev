...
function buildMinutesUI() {
   ...
   <div class="flex flex-col h-full w-full relative">
       <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-3 shrink-0 flex flex-col md:flex-row justify-between items-center z-10 shadow-sm rounded-t-xl md:rounded-none gap-3">
           <div class="flex items-center gap-2 w-full md:w-auto">
               <h3 class="text-sm md:text-base font-black text-zinc-900 dark:text-white tracking-tight">Meeting Notes</h3>
               <span id="minutesSyncStatus" class="ml-2 text-[9px] font-bold text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded shadow-inner uppercase tracking-wider">Synced</span>
           </div>
           
           <div class="flex w-full md:w-auto items-center gap-2 justify-end">
               <div class="relative w-full md:w-48 shrink-0">
                   <input type="text" id="minutesSearchInput" oninput="handleMinutesSearch()" placeholder="Fuzzy search notes..." class="w-full p-2 pl-8 border border-zinc-300 dark:border-zinc-700 rounded-md text-xs font-semibold bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary shadow-sm transition">
                   <svg class="w-3.5 h-3.5 absolute left-2.5 top-2.5 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
               </div>
               <button onclick="addMinuteNote()" class="w-auto bg-primary text-white text-[10px] md:text-xs font-bold px-3 py-2 rounded-md hover:bg-blue-600 transition flex items-center justify-center shadow-sm focus:outline-none shrink-0 transform active:scale-95">
       ...
       <div id="minutesLoadingOverlay" class="absolute inset-0 bg-white/60 dark:bg-zinc-950/60 backdrop-blur-sm z-[50] flex flex-col justify-center items-center hidden-force">
           ...
           <span class="text-primary dark:text-blue-400 font-bold text-[10px] tracking-wide shadow-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-1 rounded-full">Loading Notes...</span>
       </div>

       <div class="flex-grow overflow-y-auto p-2 md:p-4 bg-zinc-50 dark:bg-zinc-950 custom-scrollbar pb-10">
...
function renderAllMinutes() {
   ...
   if (sorted.length === 0) {
       if (minutesSearchQuery) {
           container.innerHTML = `<div class="empty-notes-msg w-full py-10 flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-500"><p class="text-xs font-bold uppercase tracking-widest">No matching notes found.</p></div>`;
       } else {
           container.innerHTML = `<div class="empty-notes-msg w-full py-10 flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-500"><svg class="w-12 h-12 mb-3 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg><p class="text-xs font-bold uppercase tracking-widest">No meeting notes found</p></div>`;
       }
       return;
   }
...
function createNoteDOM(note) {
   ...
   div.className = "bg-white dark:bg-zinc-800 rounded-xl shadow-[0_2px_8px_-2px_rgba(0,0,0,0.1)] border border-zinc-200 dark:border-zinc-700 overflow-hidden transition-all duration-300 transform";
   
   const timeStr = note.ts ? new Date(note.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'New';
   
   div.innerHTML = `
       <div class="flex justify-between items-center bg-zinc-50/80 dark:bg-zinc-900/50 p-2 md:p-3 border-b border-zinc-100 dark:border-zinc-700 shrink-0">
           <div class="flex items-center gap-2">
               <input type="date" value="${formatYMD(note.date)}" 
                   class="note-date min-w-[120px] [color-scheme:light] dark:[color-scheme:dark] text-xs font-bold text-zinc-700 dark:text-zinc-200 bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded px-2 py-1 shadow-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition"
                   onchange="handleMinuteInput('${note.id}', 'date', this.value)">
               <span id="min-by-${note.id}" class="note-meta text-[9px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide hidden md:inline-block">${timeStr} by ${note.updatedBy}</span>
           </div>
           <button onclick="deleteMinuteNote('${note.id}')" class="text-zinc-400 hover:text-red-500 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 p-1.5 rounded shadow-sm hover:bg-red-50 dark:hover:bg-zinc-700 transition focus:outline-none">
       ...
       <div class="p-3">
           <textarea 
               class="note-content w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-lg p-3 text-xs md:text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary resize-y shadow-inner min-h-[100px]"
       ...
       <div class="bg-blue-50/50 dark:bg-blue-900/10 p-2 md:p-3 border-t border-blue-100 dark:border-blue-900/30 flex items-center gap-2">
           ...
           <input type="text" value="${note.assignedTo}" 
               class="note-assigned w-full bg-white dark:bg-zinc-800 border border-blue-200 dark:border-blue-800/50 rounded-md px-2 py-1.5 text-xs font-bold text-blue-800 dark:text-blue-300 placeholder-blue-300 dark:placeholder-blue-700 focus:outline-none focus:ring-1 focus:ring-primary shadow-sm"
   ...
   if (state === 'saving') {
       ...
       setTimeout(() => {
           if (pendingMinutesUpdates.size === 0) {
               el.className = "ml-2 text-[9px] font-bold text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded shadow-inner uppercase tracking-wider transition-colors";
           }
       }, 2000);
   } else if (state === 'error') {
