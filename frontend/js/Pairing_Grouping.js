...
function setSyncButtonState(state) {
   document.querySelectorAll('.btn-sync-pairings').forEach(btn => {
       ...
       spinner.className = "btn-spinner ml-1 !w-3 !h-3 hidden-force border-2"; 
       if (state === 'loading') { 
   ...
   <div id="log-pairings" class="flex-1 flex flex-col min-h-0 w-full relative">
       ...
       <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-2 md:p-3 shrink-0 z-10 flex flex-col gap-1 shadow-sm">
           <div class="flex justify-between items-center px-1">
               ...
               <button onclick="manualSyncPairings(this)" class="btn-sync-pairings text-[10px] md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-sm bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0"><span class="btn-text">Saved</span><div class="btn-spinner ml-1 !w-3 !h-3 hidden-force border-2"></div></button>
           </div>
       </div>
       <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative bg-zinc-50 dark:bg-zinc-950 border-x border-b border-zinc-200 dark:border-zinc-800 rounded-b-xl md:rounded-none">
           <div id="dnd-source-col" class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-r border-zinc-200 dark:border-zinc-800">
               <h4 id="dnd-source-title" class="font-black text-[10px] py-1.5 shrink-0 text-center uppercase tracking-widest border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400"></h4>
           ...
           <div id="dnd-target-col" class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors">
               <h4 id="dnd-target-title" class="font-black text-[10px] py-1.5 shrink-0 text-center uppercase tracking-widest border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400"></h4>
   ...
   <div id="log-rooms" class="hidden-force flex-1 flex flex-col min-h-0 w-full relative">
       <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-2 md:p-3 shrink-0 z-10 flex flex-col gap-2 shadow-sm">
           <div class="flex justify-between items-center px-1">
               ...
               <button id="btn-sync-rooms" onclick="manualSyncRooms(this)" class="text-[10px] md:text-xs px-2 py-1 rounded-md font-bold transition flex items-center justify-center border shadow-sm bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0"><span class="btn-text">Saved</span><div class="btn-spinner ml-1 !w-3 !h-3 hidden-force border-2"></div></button>
           </div>
           <div class="flex justify-between items-center px-1 gap-2 mt-1">
               ...
               <select id="roomFilterSelect" onchange="renderRooms()" class="text-[10px] font-bold bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded px-1.5 py-1.5 outline-none shrink-0 text-zinc-900 dark:text-white">
...
function renderPairings() {
   ...
   const genCards = (arr) => arr.map(item => {
       const dynColor = typeof window.getProjectColor === 'function' ? window.getProjectColor(item.group) : 'bg-zinc-100 dark:bg-zinc-800 border-zinc-300 dark:border-zinc-600 text-zinc-900 dark:text-white';
       ...
       let pairedPills = myPairings.map(pair => {
           ...
           if(pPerson) return `<div class="relative flex w-full align-top pointer-events-auto"><div class="${typeof window.getProjectColor === 'function' ? window.getProjectColor(pPerson.group) : 'bg-zinc-100 dark:bg-zinc-800'} text-[10px] md:text-[11px] pl-2 pr-6 py-1 rounded shadow-sm border font-bold opacity-90 leading-tight break-words whitespace-normal text-left w-full">${pPerson.displayName || pPerson.name}</div><div class="remove-x flex items-center justify-center w-5 h-full absolute right-0 top-0 cursor-pointer hover:text-red-500 font-bold" onclick="unpairTrainee('${pair.traineeNric}', '${pair.volNric}')">×</div></div>`;
           return '';
       }).join('');

       return `<div class="dnd-draggable dnd-dropzone bg-white dark:bg-zinc-800 p-1.5 rounded-md border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col gap-1" data-nric="${item.nric}" data-role="${item.role}">
           <div class="flex justify-between items-start w-full gap-1">
               ...
               <button onclick="openPairingSheet('${item.nric}', '${item.role}')" class="text-[9px] bg-blue-50 dark:bg-zinc-700 text-blue-600 dark:text-blue-400 font-bold px-1.5 py-1 rounded border hover:bg-blue-100 dark:hover:bg-zinc-600 transition whitespace-nowrap focus:outline-none shrink-0 pointer-events-auto shadow-sm">${item.role === 'TRAINEE' ? '+ Vol' : '+ Trn'}</button>
           </div>
...
function renderRooms() {
   ...
   document.getElementById('roomUnassignedPool').innerHTML = filteredUnassigned.map(item => {
       const dynColor = typeof window.getProjectColor === 'function' ? window.getProjectColor(item.group) : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white border-zinc-300 dark:border-zinc-600';
       return `<div class="dnd-room-draggable bg-white dark:bg-zinc-800 p-1.5 rounded-md border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col gap-1" data-nric="${item.nric}">
       ...
       return `<div class="dnd-room-dropzone flex flex-col bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm" data-room-id="${room.id}">
           <div class="flex justify-between items-center bg-zinc-50/80 dark:bg-zinc-900/50 p-2 border-b border-zinc-100 dark:border-zinc-700 rounded-t-xl gap-2 w-full">
               ...
               <div class="flex items-center gap-1 shrink-0">
                   <button onclick="openRoomAddSheet('${room.id}')" class="text-[9px] bg-blue-50 text-blue-600 border border-blue-200 font-bold px-1.5 py-0.5 rounded hover:bg-blue-100 transition focus:outline-none dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800 dark:hover:bg-blue-900/50" ${isFull?'disabled style="opacity:0.5;"':''}>+ Add</button>
                   <button onclick="deleteRoom('${room.id}')" class="text-zinc-400 hover:text-red-500 p-0.5 bg-white dark:bg-zinc-800 border dark:border-zinc-600 rounded shadow-sm">✕</button>
               </div>
           </div>
...
function openPairingSheet(sourceNric, sourceRole) {
   ...
   document.getElementById('sheetListContainer').innerHTML = targets.map(t => {
       ...
       if(isPaired) return ''; 
       return `<div onclick="confirmPairing('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-zinc-800 p-2.5 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-pointer hover:border-primary transition mb-1.5" data-name="${(t.displayName||t.name).toLowerCase()}"><span class="font-extrabold text-[11px] ${typeof window.getProjectColor === 'function' ? window.getProjectColor(t.group) : 'bg-zinc-100'} px-1.5 py-0.5 rounded shadow-sm border">${t.displayName||t.name}</span></div>`;
...
function openRoomAddSheet(roomId) {
   ...
   document.getElementById('sheetListContainer').innerHTML = unassignedArr.map(t => `<div onclick="confirmRoomAdd('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-zinc-800 p-2.5 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-pointer hover:border-primary transition mb-1.5" data-name="${(t.displayName||t.name).toLowerCase()}"><span class="font-extrabold text-[11px] ${typeof window.getProjectColor === 'function' ? window.getProjectColor(t.group) : 'bg-zinc-100'} px-1.5 py-0.5 rounded shadow-sm border">${t.displayName||t.name}</span></div>`).join('') || `<p class="text-[10px] font-bold text-zinc-400 p-2 text-center mt-2">Everyone is assigned.</p>`;
}
