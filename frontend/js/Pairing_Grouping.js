...
if (!window.dndInitialized) {
  ...
  function moveDrag(e, clientX, clientY, isTouch) {
      ...
      if (!dndState.isDragging) {
          ...
          if ((dndState.type === 'pairing' && deltaX > threshold && deltaX > deltaY) || (dndState.type === 'rooming' && (deltaX > threshold || deltaY > threshold))) {
              ...
              dndState.el.classList.add('locked-for-drag', 'opacity-50');
              dndState.clone = dndState.nameNode.cloneNode(true);
              dndState.clone.classList.add('dragging-clone', 'fixed', 'z-[9999]', 'pointer-events-none', 'shadow-2xl');
          ...
          if (dndState.type === 'pairing') {
              ...
              document.querySelectorAll('.dnd-dropzone').forEach(dz => {
                  if (dz === activeDz && dz.dataset.role !== dndState.el.dataset.role) {
                      dz.classList.add('border-primary', 'bg-blue-50/50', 'dark:bg-zinc-800', 'ring-2', 'ring-primary');
                  } else {
                      dz.classList.remove('border-primary', 'bg-blue-50/50', 'dark:bg-zinc-800', 'ring-2', 'ring-primary');
                  }
              });
              ...
              document.querySelectorAll('.dnd-room-dropzone').forEach(dz => {
                  if (dz === activeRoom) dz.classList.add('border-primary', 'bg-blue-50/50', 'dark:bg-zinc-800', 'ring-2', 'ring-primary');
                  else dz.classList.remove('border-primary', 'bg-blue-50/50', 'dark:bg-zinc-800', 'ring-2', 'ring-primary');
              });
          }
      }
  }
  ...
  function endDrag(e, clientX, clientY) {
      if(dndState.el) dndState.el.classList.remove('locked-for-drag', 'opacity-50');
      if (dndState.isDragging && dndState.clone) {
          dndState.clone.remove(); dndState.clone = null; dndState.isDragging = false;
          document.querySelectorAll('.dnd-dropzone, .dnd-room-dropzone').forEach(dz => dz.classList.remove('border-primary', 'bg-blue-50/50', 'dark:bg-zinc-800', 'ring-2', 'ring-primary'));
...
function setSyncButtonState(state) {
  document.querySelectorAll('.btn-sync-pairings').forEach(btn => {
      ...
      btn.className = "btn-sync-pairings text-xs md:text-sm px-3 py-1.5 rounded-lg font-bold transition flex items-center justify-center shadow-sm focus:outline-none shrink-0 border transform active:scale-95"; 
...
function buildLogisticsUI() {
  ...
  <div class="flex flex-col h-full w-full relative bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      ...
      <div class="flex overflow-x-auto bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 scrollbar-hide shrink-0 px-4 pt-2 z-10 gap-2">
          <button onclick="switchLogisticsSubTab('pairings')" id="subTab-pairings" class="px-4 py-2.5 font-bold border-b-2 border-primary text-primary whitespace-nowrap text-sm transition focus:outline-none flex items-center gap-2"><i class="fa-solid fa-people-arrows"></i> Pairings</button>
          <button onclick="switchLogisticsSubTab('rooms')" id="subTab-rooms" class="px-4 py-2.5 font-bold border-b-2 border-transparent text-zinc-500 dark:text-zinc-400 whitespace-nowrap text-sm transition focus:outline-none hover:text-zinc-800 dark:hover:text-zinc-200 flex items-center gap-2"><i class="fa-solid fa-door-open"></i> Rooms</button>
          <button onclick="switchLogisticsSubTab('groups')" id="subTab-groups" class="px-4 py-2.5 font-bold border-b-2 border-transparent text-zinc-500 dark:text-zinc-400 whitespace-nowrap text-sm transition focus:outline-none hover:text-zinc-800 dark:hover:text-zinc-200 flex items-center gap-2"><i class="fa-solid fa-users"></i> Groups</button>
      </div>

      <!-- Pairings -->
      <div id="log-pairings" class="flex-1 flex flex-col min-h-0 w-full relative bg-zinc-50 dark:bg-black">
          <div id="logLoadingOverlay" class="absolute inset-0 bg-white/60 dark:bg-zinc-950/60 backdrop-blur-sm z-[50] hidden-force flex flex-col justify-center items-center">
          ...
          <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-3 md:p-4 shrink-0 z-10 flex flex-col gap-2 shadow-sm">
              <div class="flex justify-between items-center">
                  <div class="flex items-center gap-3">
                      <h3 class="text-base md:text-lg font-black text-zinc-900 dark:text-white tracking-tight">Manual Pairing</h3>
                      <button onclick="toggleAltSwap()" class="bg-zinc-100 dark:bg-zinc-800 w-8 h-8 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition focus:outline-none border border-zinc-200 dark:border-zinc-700 shadow-sm flex items-center justify-center transform active:scale-95" title="Swap Columns">
                          <i class="fa-solid fa-right-left text-zinc-700 dark:text-zinc-300"></i>
                      </button>
                  </div>
                  <button onclick="manualSyncPairings(this)" class="btn-sync-pairings text-xs md:text-sm px-3 py-1.5 rounded-lg font-bold transition flex items-center justify-center border shadow-sm bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0 transform active:scale-95"><span class="btn-text">Saved</span><div class="btn-spinner ml-1.5 !w-3.5 !h-3.5 hidden-force border-2"></div></button>
              </div>
          </div>
          <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative">
              <div id="dnd-source-col" class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-r border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-900/10">
                  <h4 id="dnd-source-title" class="font-black text-[10px] md:text-xs py-2.5 shrink-0 text-center uppercase tracking-widest border-b border-green-200 dark:border-green-800 bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-400"></h4>
                  <div id="dnd-source-pool" class="space-y-2 flex-grow overflow-y-auto p-2 md:p-3 custom-scrollbar pb-10"></div>
              </div>
              <div id="dnd-target-col" class="flex-1 min-w-0 flex flex-col h-full overflow-hidden transition-colors border-l border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/10">
                  <h4 id="dnd-target-title" class="font-black text-[10px] md:text-xs py-2.5 shrink-0 text-center uppercase tracking-widest border-b border-blue-200 dark:border-blue-800 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-400"></h4>
                  <div id="dnd-target-list" class="space-y-2 flex-grow overflow-y-auto p-2 md:p-3 custom-scrollbar pb-10"></div>
              </div>
          </div>
      </div>

      <!-- Rooms -->
      <div id="log-rooms" class="hidden-force flex-1 flex flex-col min-h-0 w-full relative bg-zinc-50 dark:bg-black">
          <div class="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-3 md:p-4 shrink-0 z-10 flex flex-col gap-3 shadow-sm">
              <div class="flex justify-between items-center">
                  <div class="flex flex-wrap items-center gap-2">
                      <h3 class="text-base md:text-lg font-black text-zinc-900 dark:text-white mr-2 shrink-0">Room Assignment</h3>
                      <button onclick="resetRoomAssignments()" class="bg-orange-50 text-orange-600 border border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800 text-[10px] md:text-xs font-bold px-2 py-1.5 rounded-lg shadow-sm hover:bg-orange-100 transition focus:outline-none transform active:scale-95">Unassign</button>
                      <button onclick="deleteAllRooms()" class="bg-red-50 text-red-600 border border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800 text-[10px] md:text-xs font-bold px-2 py-1.5 rounded-lg shadow-sm hover:bg-red-100 transition focus:outline-none transform active:scale-95">Delete All</button>
                      <button onclick="autoAssignRooms()" class="bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800 text-[10px] md:text-xs font-bold px-2 py-1.5 rounded-lg shadow-sm hover:bg-blue-100 transition focus:outline-none transform active:scale-95">Auto-Room</button>
                      <button onclick="addRoom()" class="bg-zinc-100 dark:bg-zinc-800 w-8 h-8 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition focus:outline-none border border-zinc-200 dark:border-zinc-700 shadow-sm shrink-0 flex items-center justify-center transform active:scale-95" title="Add Room(s)"><i class="fa-solid fa-plus text-zinc-700 dark:text-zinc-300"></i></button>
                  </div>
                  <button id="btn-sync-rooms" onclick="manualSyncRooms(this)" class="text-xs md:text-sm px-3 py-1.5 rounded-lg font-bold transition flex items-center justify-center border shadow-sm bg-green-50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800 focus:outline-none shrink-0 transform active:scale-95"><span class="btn-text">Saved</span><div class="btn-spinner ml-1.5 !w-3.5 !h-3.5 hidden-force border-2"></div></button>
              </div>
              <div class="flex justify-between items-center gap-3">
                  <div class="relative w-full max-w-sm">
                      <input type="text" id="roomSearchInput" oninput="renderRooms()" placeholder="Fuzzy search..." class="w-full p-2 pl-8 border border-zinc-300 dark:border-zinc-700 rounded-lg text-xs font-bold bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-sm transition">
                      <i class="fa-solid fa-magnifying-glass absolute left-3 top-2.5 text-zinc-400"></i>
                  </div>
                  <select id="roomFilterSelect" onchange="renderRooms()" class="text-xs font-bold bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-lg px-2 py-2 outline-none shrink-0 text-zinc-900 dark:text-white shadow-sm cursor-pointer transition">
          ...
          <div class="flex flex-row flex-1 min-h-0 w-full overflow-hidden relative">
              <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden border-r border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/50">
                  <h4 class="font-black text-[10px] md:text-xs py-2.5 shrink-0 text-center uppercase tracking-widest bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-400 border-b border-zinc-200 dark:border-zinc-700">Unassigned (<span id="unassignedCount">0</span>)</h4>
                  <div id="roomUnassignedPool" class="space-y-2 flex-grow overflow-y-auto p-2 md:p-3 custom-scrollbar pb-10"></div>
              </div>
              <div class="flex-1 min-w-0 flex flex-col h-full overflow-hidden relative">
                  <div id="roomListContainer" class="flex-grow overflow-y-auto p-2 md:p-3 custom-scrollbar flex flex-col gap-3 md:gap-4 pb-10"></div>
              </div>
          </div>
      </div>
      
      <div id="log-groups" class="hidden-force flex-1 w-full bg-zinc-50 dark:bg-black p-6">
          <div class="bg-white dark:bg-zinc-900 p-8 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center text-center max-w-lg mx-auto mt-10">
              <div class="w-16 h-16 bg-orange-100 dark:bg-orange-900/30 text-orange-500 rounded-full flex items-center justify-center text-3xl mb-4"><i class="fa-solid fa-person-digging"></i></div>
              <h3 class="text-lg font-black text-zinc-900 dark:text-white mb-2">Group Builder</h3>
              <p class="text-sm text-zinc-500 dark:text-zinc-400">The advanced grouping engine is currently under development. Check back later for updates.</p>
          </div>
      </div>
  </div>
  `;
...
function renderPairings() {
  ...
  const genCards = (arr) => arr.map(item => {
      const dynColor = typeof window.getProjectColor === 'function' ? window.getProjectColor(item.group) : 'bg-zinc-100 border-zinc-300 text-zinc-900 dark:bg-zinc-800 dark:border-zinc-600 dark:text-zinc-100';
      ...
      let pairedPills = myPairings.map(pair => {
          ...
          if(pPerson) return `<div class="relative flex w-full align-top pointer-events-auto"><div class="${typeof window.getProjectColor === 'function' ? window.getProjectColor(pPerson.group) : 'bg-zinc-100 dark:bg-zinc-800'} text-[11px] md:text-xs pl-2.5 pr-8 py-1.5 rounded-lg shadow-sm border font-bold opacity-95 leading-tight break-words whitespace-normal text-left w-full">${pPerson.displayName || pPerson.name}</div><div class="remove-x flex items-center justify-center w-7 h-full absolute right-0 top-0 cursor-pointer hover:text-red-500 font-bold transition-transform hover:scale-110" onclick="unpairTrainee('${pair.traineeNric}', '${pair.volNric}')"><i class="fa-solid fa-xmark"></i></div></div>`;
          return '';
      }).join('');

      return `<div class="dnd-draggable dnd-dropzone bg-white dark:bg-zinc-900 p-2 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col gap-1.5" data-nric="${item.nric}" data-role="${item.role}">
          <div class="flex justify-between items-start w-full gap-2">
              <div class="main-name-pill font-extrabold text-xs px-2 py-1 rounded-md border shadow-sm ${dynColor} max-w-full leading-tight"><span class="break-words whitespace-normal">${item.displayName || item.name}</span></div>
              <button onclick="openPairingSheet('${item.nric}', '${item.role}')" class="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 font-bold px-2 py-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-700 hover:text-primary dark:hover:text-blue-400 transition whitespace-nowrap focus:outline-none shrink-0 pointer-events-auto shadow-sm transform active:scale-95"><i class="fa-solid fa-plus"></i> ${item.role === 'TRAINEE' ? 'Vol' : 'Trn'}</button>
          </div>
          <div class="flex flex-col pointer-events-auto bg-zinc-50/80 dark:bg-zinc-950/50 p-2 rounded-lg min-h-[44px] border border-dashed border-zinc-300 dark:border-zinc-700 w-full gap-1.5">
              ${pairedPills || '<div class="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 text-center py-1.5 uppercase tracking-widest"><i class="fa-solid fa-link opacity-50 mr-1"></i>Drop here</div>'}
          </div>
      </div>`;
  }).join('');

  document.getElementById('dnd-source-pool').innerHTML = genCards(sourceArr) || '<p class="text-[10px] text-zinc-400 font-bold p-4 text-center mt-2 uppercase tracking-widest"><i class="fa-solid fa-ghost opacity-50 text-2xl block mb-2"></i>Empty</p>';
  document.getElementById('dnd-target-list').innerHTML = genCards(targetArr) || '<p class="text-[10px] text-zinc-400 font-bold p-4 text-center mt-2 uppercase tracking-widest"><i class="fa-solid fa-ghost opacity-50 text-2xl block mb-2"></i>Empty</p>';
}
...
function renderRooms() {
  ...
  document.getElementById('roomUnassignedPool').innerHTML = filteredUnassigned.map(item => {
      const dynColor = typeof window.getProjectColor === 'function' ? window.getProjectColor(item.group) : 'bg-zinc-100 border-zinc-300 text-zinc-900 dark:bg-zinc-800 dark:border-zinc-600 dark:text-zinc-100';
      return `<div class="dnd-room-draggable bg-white dark:bg-zinc-900 p-2 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-grab active:cursor-grabbing hover:border-primary dark:hover:border-primary transition select-none flex flex-col gap-1" data-nric="${item.nric}">
          <div class="main-name-pill font-extrabold text-xs px-2.5 py-1.5 rounded-md border shadow-sm ${dynColor} w-full flex items-start justify-between gap-2"><span class="break-words whitespace-normal text-left flex-1">${item.displayName || item.name}</span></div>
      </div>`;
  }).join('') || '<div class="text-center text-[10px] font-bold text-zinc-400 p-4 mt-2 uppercase tracking-widest"><i class="fa-solid fa-ghost opacity-50 text-2xl block mb-2"></i>All Assigned</div>';
  ...
  document.getElementById('roomListContainer').innerHTML = roomsToRender.map(room => {
      ...
      let occHtml = room.occupants.map(nric => {
          ...
          if(p) return `<div class="dnd-room-draggable relative flex w-full cursor-grab active:cursor-grabbing hover:scale-[1.02] transition-transform pointer-events-auto" data-nric="${p.nric}"><div class="main-name-pill ${typeof window.getProjectColor === 'function' ? window.getProjectColor(p.group) : 'bg-zinc-100'} text-[11px] md:text-xs px-2.5 py-2 rounded-lg shadow-sm border font-bold opacity-95 leading-tight w-full pr-7">${p.displayName || p.name}</div><div class="remove-x flex items-center justify-center w-7 h-full absolute right-0 top-0 cursor-pointer hover:text-red-500 font-bold transition-transform hover:scale-110" onclick="unassignFromRoom('${p.nric}', '${room.id}')"><i class="fa-solid fa-xmark"></i></div></div>`;
          return '';
      }).join('');

      return `<div class="dnd-room-dropzone flex flex-col bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-md transition-colors duration-200" data-room-id="${room.id}">
          <div class="flex justify-between items-center bg-zinc-50 dark:bg-zinc-800 p-3 md:p-4 border-b border-zinc-200 dark:border-zinc-700 rounded-t-2xl gap-2 w-full">
              <div class="flex flex-col min-w-0">
                  <span class="font-black text-sm md:text-base text-zinc-900 dark:text-white break-words leading-tight truncate">${room.name}</span>
                  <span class="text-[10px] md:text-xs font-bold ${isFull ? 'text-red-500 dark:text-red-400' : 'text-zinc-500 dark:text-zinc-400'} uppercase tracking-wider mt-0.5">${room.occupants.length} / ${room.capacity} Occupants</span>
              </div>
              <div class="flex items-center gap-1.5 shrink-0">
                  <button onclick="openRoomAddSheet('${room.id}')" class="text-xs bg-blue-50 text-blue-600 border border-blue-200 font-bold px-3 py-1.5 rounded-lg hover:bg-blue-100 transition focus:outline-none dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800 dark:hover:bg-blue-900/50 shadow-sm transform active:scale-95 flex items-center" ${isFull?'disabled style="opacity:0.5;"':''}><i class="fa-solid fa-plus mr-1"></i> Add</button>
                  <button onclick="deleteRoom('${room.id}')" class="text-zinc-400 hover:text-red-500 w-8 h-8 flex items-center justify-center bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-600 rounded-lg shadow-sm transition hover:bg-red-50 dark:hover:bg-zinc-700 transform active:scale-95"><i class="fa-solid fa-trash text-sm"></i></button>
              </div>
          </div>
          <div class="p-2 md:p-3 min-h-[50px] flex flex-col pointer-events-auto bg-transparent gap-2 w-full">${occHtml || '<div class="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 text-center py-3 uppercase tracking-widest"><i class="fa-solid fa-arrow-down opacity-50 mr-1.5"></i>Drop here</div>'}</div>
      </div>`;
  }).join('') || '<div class="text-center text-xs font-bold text-zinc-400 uppercase tracking-widest mt-10"><i class="fa-solid fa-ghost text-4xl block mb-3 opacity-50"></i>No rooms found.</div>';
}
...
function openPairingSheet(sourceNric, sourceRole) {
  ...
  document.getElementById('sheetTitle').innerHTML = `Pair with <span class="text-primary">${sourcePerson ? sourcePerson.displayName||sourcePerson.name : ''}</span>`;
  ...
  document.getElementById('sheetListContainer').innerHTML = targets.map(t => {
      ...
      if(isPaired) return ''; 
      return `<div onclick="confirmPairing('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-zinc-800 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-pointer hover:border-primary dark:hover:border-primary transition mb-2" data-name="${(t.displayName||t.name).toLowerCase()}"><span class="font-extrabold text-xs md:text-sm ${typeof window.getProjectColor === 'function' ? window.getProjectColor(t.group) : 'bg-zinc-100'} px-2.5 py-1.5 rounded-lg shadow-sm border truncate">${t.displayName||t.name}</span></div>`;
  }).join('') || `<div class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest p-6 text-center mt-4 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl">No available options.</div>`;
}
...
function openRoomAddSheet(roomId) {
  ...
  document.getElementById('sheetListContainer').innerHTML = unassignedArr.map(t => `<div onclick="confirmRoomAdd('${t.nric}')" class="sheet-list-item flex flex-col bg-white dark:bg-zinc-800 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm cursor-pointer hover:border-primary dark:hover:border-primary transition mb-2" data-name="${(t.displayName||t.name).toLowerCase()}"><span class="font-extrabold text-xs md:text-sm ${typeof window.getProjectColor === 'function' ? window.getProjectColor(t.group) : 'bg-zinc-100'} px-2.5 py-1.5 rounded-lg shadow-sm border truncate">${t.displayName||t.name}</span></div>`).join('') || `<div class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest p-6 text-center mt-4 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl"><i class="fa-solid fa-ghost block text-2xl mb-2 opacity-50"></i>Everyone is assigned.</div>`;
}
