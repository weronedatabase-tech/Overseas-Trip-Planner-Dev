...
document.addEventListener('touchmove', onMedMouseMove, {passive: false});
document.addEventListener('touchend', onMedMouseUp);
}
...
document.removeEventListener('touchmove', onMedMouseMove);
document.removeEventListener('touchend', onMedMouseUp);
}
...
let headHtml = `<tr>
   <th class="p-3 relative bg-gray-100 dark:bg-gray-800 med-col-fullName align-top" style="width: 250px; min-width: 250px; max-width: 250px;" data-col-id="fullName">
       ...
       <div class="resize-handle" onmousedown="initMedResize(event, 'fullName')" ontouchstart="initMedResize(event, 'fullName')"></div>
   </th>`;
...
medCols.forEach(c => {
   if (c.visible) {
       ...
       <th class="p-3 relative bg-gray-100 dark:bg-gray-800 med-col-${c.id} align-top" 
           ...
           <div class="resize-handle" onmousedown="initMedResize(event, '${c.id}')" ontouchstart="initMedResize(event, '${c.id}')"></div>
       </th>`;
   }
});
headHtml += `</tr>`;
