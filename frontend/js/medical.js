...
{ id: 'medical', label: 'Medical & Medications', width: 260, visible: true },
...
if (!medCols.find(c => c.id === 'medical')) {
    medCols.splice(1, 0, { id: 'medical', label: 'Medical & Medications', width: 260, visible: true });
   ...
   medCols.forEach(c => {
       if (c.visible) {
           ...
           if (c.id === 'diet') {
               ...
               const medicalHtml = hasMedical ? `<span class="text-rose-700 dark:text-rose-400 font-bold bg-rose-50 dark:bg-rose-900/20 px-2 py-1 rounded inline-block whitespace-pre-wrap">${p.medical}</span>` : `<span class="text-gray-400 dark:text-gray-600 italic">NONE</span>`;
               html += `<td class="${baseClass} border-l border-gray-100 dark:border-gray-800/50" ${styleStr}>${medicalHtml}</td>`;
