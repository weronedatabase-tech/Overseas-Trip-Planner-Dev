const fs = require('fs');
let code = fs.readFileSync('frontend/js/diet.js', 'utf8');

// Replace renderDietTable
const renderDietTableStr = `function renderMedicalTable() {
...
code = code.replace(/function renderMedicalTable\(\) \{[\s\S]*?(?=\n\}\n?)$/m, renderDietTableStr + "\n");
fs.writeFileSync('frontend/js/diet.js', code);
console.log("Patched diet.js");
