...
document.getElementById('tab-settings').innerHTML = `
 ...
 <div class="bg-white dark:bg-gray-900 p-3 md:p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 main-admin-only ${currentUser && currentUser.nric === 'ADMIN' ? '' : 'hidden-force'}">
   <h3 class="text-sm font-black text-gray-900 dark:text-white mb-0.5 tracking-tight">Drive Access Management</h3>
   <p class="text-[9px] font-semibold text-gray-500 dark:text-gray-400 mb-3 leading-tight">Grant specific users access to the underlying Google Drive trip folder. You can only revoke access that was granted via this interface.</p>
   <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-3 mt-2">
     <input type="email" id="newDriveEmail" placeholder="Google Account Email" class="flex-grow p-2 border border-gray-300 dark:border-gray-700 rounded-md text-xs font-semibold bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-primary shadow-sm">
     <select id="newDriveRole" class="p-2 border border-gray-300 dark:border-gray-700 rounded-md text-xs font-bold bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white w-full md:w-28 focus:outline-none focus:ring-1 focus:ring-primary shadow-sm">
       <option value="viewer">Viewer</option>
       <option value="editor">Editor</option>
     </select>
     <button onclick="addDriveAccessBtn(this)" class="bg-primary text-white px-3 py-2 text-xs rounded-md font-bold flex items-center justify-center shadow-sm shrink-0"><span class="btn-text">Add</span><div class="btn-spinner spinner-white hidden-force ml-1.5 !w-3 !h-3 border-2"></div></button>
   </div>
